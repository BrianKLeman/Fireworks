...
sampler2D gForegroundSampler = sampler_state {
    texture = (gForegroundTexture);
    AddressU = Clamp;
    AddressV = Clamp;
#if DIRECT3D_VERSION >= 0xa00
...
{   
    ...
    float4 glow = Glowness * tex2D(GlowSampler, IN.UV);
	float m = max(max(glow.r,glow.b),glow.g);
	m /= (m > 1.0 ? m : 1.0);
    return float4(scn.xyz + glow.xyz, m);
}  

float4 CompositePS(OneTexelVertex IN,
	uniform sampler2D ScnSampler,
	uniform sampler2D GlowSampler
) : COLOR
{   
    float4 scn = tex2D(ScnSampler, IN.UV);
    float4 glow = tex2D(GlowSampler, IN.UV);
