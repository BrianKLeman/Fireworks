#include "YellowRocket.h"

CYellowRocket::CYellowRocket(vector<CAbstractParticle*>* pManagersLeftOverParticles,const D3DXVECTOR3& pos,const D3DXVECTOR3& vel,const D3DXVECTOR3& acc,const D3DXVECTOR3& thrust_,float energy_,float life_time)
:CRocket(pManagersLeftOverParticles,pos,vel,acc,thrust_,energy_,life_time)
{

}

CYellowRocket::~CYellowRocket(void)
{
}

void CYellowRocket::update()
{

	
	
	if((timer->getTimeElapsedFromStart() - time_born) > (life_length)) { alive = false; }
	...
	if((timer->getTimeElapsedFromStart()-time_born)<(life_length*0.6f) ) //quarter of the time to explosion
	{
...
vector<CAbstractParticle>* CYellowRocket::command1() //called on event 1 i.e. explode into more particles
{
